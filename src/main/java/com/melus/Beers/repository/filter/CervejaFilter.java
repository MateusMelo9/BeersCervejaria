package com.melus.Beers.repository.filter;

import java.math.BigDecimal;

import com.melus.Beers.model.Estilo;
import com.melus.Beers.model.Origem;
import com.melus.Beers.model.Sabor;

public class CervejaFilter {

	private String sku;
	private String nome;
	private Estilo estilo;
	private Sabor sabor;
	private Origem origem;
	private BigDecimal precoInit;
	private BigDecimal precoFim;
	
	public String getSku() {
		return sku;
	}
	public void setSku(String sku) {
		this.sku = sku;
	}
	public String getNome() {
		return nome;
	}
	public void setNome(String nome) {
		this.nome = nome;
	}
	public Estilo getEstilo() {
		return estilo;
	}
	public void setEstilo(Estilo estilo) {
		this.estilo = estilo;
	}
	public Sabor getSabor() {
		return sabor;
	}
	public void setSabor(Sabor sabor) {
		this.sabor = sabor;
	}
	public Origem getOrigem() {
		return origem;
	}
	public void setOrigem(Origem origem) {
		this.origem = origem;
	}
	public BigDecimal getPrecoInit() {
		return precoInit;
	}
	public void setPrecoInit(BigDecimal precoInit) {
		this.precoInit = precoInit;
	}
	public BigDecimal getPrecoFim() {
		return precoFim;
	}
	public void setPrecoFim(BigDecimal precoFim) {
		this.precoFim = precoFim;
	}
	
	
}
